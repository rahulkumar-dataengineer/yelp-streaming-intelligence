...
### Deployment Steps (executed on GCP VM via SSH)

1. **Copy artifacts** — `gcloud compute scp` to transfer `build/`, `config/`, `agents/`, `utils/`, `api.py`, `graph.py`, `requirements.txt`, `.env`, `gcp-key.json`
2. **Qdrant migration** — existing standalone Qdrant container data was at `/home/rahul/qdrant_storage`. Stopped the container, updated compose volume to a bind mount at the same path. `yelp_reviews` collection survived intact.
3. **Install Docker Compose plugin** — VM had Docker 28.2 but no compose plugin; installed via `curl` from GitHub releases (`v5.1.0`).
4. **Directory nesting fix** — `gcloud compute scp --recurse` created nested directories (`config/config/`, `agents/agents/`, `utils/utils/`). Flattened with `mv` before rebuild.
5. **Build and start** — `docker compose -f build/docker-compose.yml up -d --build`. Both containers running: `build-qdrant-1` and `build-api-1`.
6. **nginx + HTTPS** — No custom domain. Used `sslip.io` (free DNS service: `34-10-46-213.sslip.io` resolves to the VM IP). Installed nginx, obtained a Let's Encrypt cert via certbot. Auto-renews every 90 days.
7. **GCP firewall** — Opened ports 80 and 443 via `gcloud compute firewall-rules create` + tagged the VM instance with `http-server,https-server`.

### Live Endpoint
```
https://34-10-46-213.sslip.io/health
https://34-10-46-213.sslip.io/query
```

### Verified Routes
All three routes verified against the live deployed API:
- **VECTOR:** `"cozy Italian restaurants in Phoenix"` → routed correctly, Qdrant searched
- **SQL:** `"average rating for restaurants in Scottsdale"` → BigQuery queried, honest "no data" answer
- **HYBRID:** `"best restaurants in the top rated cities"` → SQL identified top cities + business IDs, vector matched semantically, synthesizer combined both with specific restaurant names and review quotes
